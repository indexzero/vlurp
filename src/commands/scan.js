...
export function ScanCommand({targetPath}) {
  ...
  for (const result of fileResults) {
    elements.push(React.createElement(Text, {key: `file-${result.file}`, bold: true}, `  ${result.file}`));

    // Injection (high severity)
    if (result.injection.length > 0) {
      for (const inj of result.injection) {
        elements.push(React.createElement(Text, {key: `inj-${result.file}-${inj.label}`, color: 'red'}, `    ISSUE ${inj.label} (${inj.count} instance${inj.count > 1 ? 's' : ''})`));
      }
    } else {
      elements.push(React.createElement(Text, {key: `ok-inj-${result.file}`, color: 'green'}, '    ok   No injection patterns detected'));
    }

    // Exfiltration
    for (const exf of result.exfiltration) {
      elements.push(React.createElement(Text, {key: `exf-${result.file}-${exf}`, color: 'red'}, `    ISSUE Exfiltration: ${exf}`));
    }

    // Escalation
    for (const esc of result.escalation) {
      elements.push(React.createElement(Text, {key: `esc-${result.file}-${esc}`, color: 'red'}, `    ISSUE Escalation: ${esc}`));
    }

    // Tool references (warnings)
    for (const [tool, count] of Object.entries(result.tools)) {
      elements.push(React.createElement(Text, {key: `tool-${result.file}-${tool}`, color: 'yellow'}, `    warn References ${tool} tool (${count} instance${count > 1 ? 's' : ''})`));
    }

    // Persistence (warnings)
    for (const per of result.persistence) {
      elements.push(React.createElement(Text, {key: `per-${result.file}-${per}`, color: 'yellow'}, `    warn Persistence: ${per}`));
    }

    // Commands
    if (result.commands.size > 0) {
      elements.push(React.createElement(Text, {key: `cmd-${result.file}`, color: 'yellow'}, `    warn References external commands: ${[...result.commands].join(', ')}`));
    }
  }
  ...
  if (summary.tool_surface.length > 0) {
    elements.push(React.createElement(Text, {key: 'tools', color: 'gray'}, `Tool surface: ${summary.tool_surface.join(', ')}`));
  }

  if (summary.command_surface.length > 0) {
    elements.push(React.createElement(Text, {key: 'commands', color: 'gray'}, `Command surface: ${summary.command_surface.join(', ')}`));
  }
